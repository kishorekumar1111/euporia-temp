...
export function EuphoriaBackground({ scrollProgress, hasEntered }: { scrollProgress: MotionValue<number>, hasEntered: boolean }) {
  ...
  const hoverSpring = useSpring(rawHover, { stiffness: 60, damping: 20 });
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => rawHover.set(1);
    const handleMouseLeave = () => rawHover.set(0);
  ...
  const orb1Scale = useTransform([clickSpring, hoverSpring], ([click, hover]) => 1 + hover * 0.05 + click * 0.25);
  const orb1Filter = useTransform([clickSpring, hoverSpring], ([click, hover]) => `blur(${click * 15}px) saturate(${1 + hover * 0.4 + click * 0.4}) brightness(${1 + hover * 0.1 + click * 0.3})`);

  const orb2X = useTransform(mouseX, v => v * 300);
  const orb2Y = useTransform(mouseY, v => v * 200);
  const orb2Scale = useTransform([clickSpring, hoverSpring], ([click, hover]) => 1 + hover * 0.05 + click * 0.3);
  const orb2Filter = useTransform([clickSpring, hoverSpring], ([click, hover]) => `blur(${click * 20}px) saturate(${1 + hover * 0.4 + click * 0.5}) brightness(${1 + hover * 0.1 + click * 0.4})`);

  const orb3X = useTransform(mouseX, v => v * -150);
  const orb3Y = useTransform(mouseY, v => v * 350);
  const orb3Scale = useTransform([clickSpring, hoverSpring], ([click, hover]) => 1 + hover * 0.05 + click * 0.2);
  const orb3Filter = useTransform([clickSpring, hoverSpring], ([click, hover]) => `blur(${click * 12}px) saturate(${1 + hover * 0.5 + click * 0.5}) brightness(${1 + hover * 0.1 + click * 0.5})`);

  const orb4X = useTransform(mouseX, v => v * 250);
  const orb4Y = useTransform(mouseY, v => v * -250);
  const orb4Scale = useTransform([clickSpring, hoverSpring], ([click, hover]) => 1 + hover * 0.05 + click * 0.35);
  const orb4Filter = useTransform([clickSpring, hoverSpring], ([click, hover]) => `blur(${click * 18}px) saturate(${1 + hover * 0.3 + click * 0.4}) brightness(${1 + hover * 0.1 + click * 0.2})`);
    ...
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0 bg-midnight transition-colors duration-[3000ms]">
      ...
      >
        <motion.div 
          style={{ 
            y: y1, 
            opacity: hasEntered ? opacityPink : 0.4,
            background: 'radial-gradient(circle, var(--color-pink) 0%, transparent 60%)'
          }}
      ...
      >
        <motion.div 
          style={{ 
            y: y2, 
            opacity: hasEntered ? opacityLavender : 0.2,
      ...
      >
        <motion.div 
          style={{ 
            y: y1, 
            opacity: hasEntered ? opacityBlue : 0,
            background: 'radial-gradient(circle, var(--color-blue) 0%, transparent 60%)'
          }}
      ...
      >
        <motion.div 
          style={{ 
            y: y2, 
            opacity: hasEntered ? opacityPeach : 0,
            background: 'radial-gradient(circle, var(--color-peach) 0%, transparent 60%)'
          }}
